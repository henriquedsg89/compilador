...
start:	{
			...
			for (LRItem item : items )
			{
				...
				{				
					...
					{
						IntegerSet prods = g.productionsFor(B);
						for (Production p2 : g.getProductions(prods) )
						{
							IntList tmp = new IntList();
							for (int i=item.getPosition()+1; i<p.get_rhs().size(); i++)
								tmp.add(p.get_rhs().get(i));
							tmp.add(item.getLookahead());
							IntegerSet first = g.first(tmp);
							for (Integer b : first)
							{
								LRItem ni = new LRItem(p2, 0, b.intValue());
	...
	protected List<List<LRItem>> computeItems()
	{
		...
		int f = sp.first();
		s.add(new LRItem(g.getProductions().get(f), 0, Grammar.DOLLAR));
